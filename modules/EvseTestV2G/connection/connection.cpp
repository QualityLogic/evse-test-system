...
ssize_t connection_read(struct v2g_connection* conn, unsigned char* buf, size_t count) {
    ...
    while ((bytes_read < count) && (is_sequence_timeout(ts_start, conn->ctx) == false) &&
           ((conn->ctx->is_connection_terminated == false) ||
            (conn->ctx->force_next_connection_req_res == true))) { // [V2G2-536]

        if (not logged_override and (conn->ctx->force_next_connection_req_res == true)) {
            dlog(DLOG_LEVEL_DEBUG, "(connection_read) Forcing connection read despite connection closure");
    ...
    if ((conn->ctx->is_connection_terminated == true) and (conn->ctx->force_next_connection_req_res == false)) {
        dlog(DLOG_LEVEL_ERROR, "Reading from tcp-socket aborted");
        return -2;
    }

    if (conn->ctx->force_next_connection_req_res == true) {
        dlog(DLOG_LEVEL_DEBUG, "(connection_read) set force_next_connection_req_res back to false");
        // conn->ctx->force_next_connection_req_res = false;
    }

    return (ssize_t)bytes_read; // [V2G2-537] read bytes are corrupted if reading from socket was interrupted
