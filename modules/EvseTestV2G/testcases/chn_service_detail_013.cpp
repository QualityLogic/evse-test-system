...
void TestBase::on_connection_close_event(v2g_connection* conn, const ConnectionCloseEvent& event) {
    ...
    // If `validate_control_pilot` is 'true' then the control pilot was not in state 'B' at the time the
    // 'PaymentServiceSelectionRes' timer expired. We must validate that the EV transitioned to state 'B'.
...
v2g_event Iso2TestServer::handle_iso_service_discovery(v2g_connection* conn) {
    ...
    if (conn->ctx->test_data.include_custom_service and
        ...
        // Define a ServiceID
        custom_service.ServiceID = SERVICE_DETAIL_013_SERVICE_ID; // 60001 – 65535 allowed for implementation specific use
...
v2g_event Iso2TestServer::handle_iso_service_detail(v2g_connection* conn) {
    ...
    const auto res = &exi_out->V2G_Message.Body.ServiceDetailRes;
    ...
    if (false == service_id_found and req->ServiceID == SERVICE_DETAIL_013_SERVICE_ID) {
        service_id_found = true;
    }
    ...
    if (next_event == V2G_EVENT_NO_EVENT and res->ResponseCode < iso2_responseCodeType_FAILED) {
        ...
        // Unset original ServiceDetailRes message
        exi_out->V2G_Message.Body.ServiceDetailRes_isUsed = 0u;
        ...
        if (next_event == V2G_EVENT_NO_EVENT and new_res->ResponseCode < iso2_responseCodeType_FAILED) {
            dlog(DLOG_LEVEL_DEBUG, "Swapped ServiceDetailRes for PaymentServiceSelectionRes");
