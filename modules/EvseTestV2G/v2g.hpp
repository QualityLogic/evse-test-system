...
#define V2G_EVCC_ONGOING_TIMEOUT_60S          60000 /* [V2G2-713] */
...
#define V2G_EVCC_CABLE_CHECK_TIMEOUT 40000 // V2G_EVCC_CableCheck_Timeout (milliseconds)
#define V2G_EVCC_PRECHARGE_TIMEOUT   7000  // V2G_EVCC_PreCharge_Timeout  (milliseconds)

/* test validation timeouts in milliseconds */
#define V2G_TEST_MESSAGE_TIMEOUT_SESSION_SETUP     2000   // V2G_EVCC_Msg_Timeout(SessionSetupReq)
#define V2G_TEST_MESSAGE_TIMEOUT_SERVICE_DETAIL    5000   // V2G_EVCC_Msg_Timeout(ServiceDetailReq)
#define V2G_TEST_EVCC_CURRENT_DEMAND_REQ_TIMEOUT    250   // par_V2G_EVCC_Msg_Timeout_CurrentDemandReq
#define V2G_TEST_EVCC_STATE_B_SHUTDOWN_TIMEOUT      4500  // par_EVCC_StateB_Shutdown_Timeout
#define V2G_TEST_TCP_CONNECTION_TERMINATION_TIMEOUT 5000  // par_CMN_TCP_Connection_Termination_Timeout
#define V2G_TEST_SCHEDULE_ENTRY_GRACE_PERIOD      120000  // 2m grace period between charge schedule entries
...
#define FORCE_PUB_MSG           25 // max msg cycles when topics values must be updated
#define MAX_PCID_LEN            17

#define DEFAULT_BUFFER_SIZE 8192
#define HEARTBEAT_UPDATE_RATE 2000 // send a heartbeat every 2s
...
struct v2g_context {
    ...
    uint32_t network_read_timeout;     /* in milliseconds */
    uint32_t network_read_timeout_tls; /* in milliseconds */
    ...
    enum v2g_protocol selected_protocol; /* Holds the selected protocol after supported app protocol */
    std::atomic<bool>
    ...
    std::atomic_bool force_next_connection_req_res;
    ...
    struct {
        std::mutex test_mutex{}; // locks all values within this struct

        /* test values */
        types::evse_test_common::TestId test_id;  // Static ID of the current test case
        int8_t supported_protocols;               // Protocols supported by the test
        bool auth_eim_supported = true; // Whether EIM is allowed for authentication
        bool auth_pnc_supported = true; // Whether PnC is allowed for authentication
        types::evse_test_common::TestOutcome outcome;
        std::vector<std::string> evidence;
        std::vector<types::test_report::Report> reports;
        std::vector<std::string> errors;

        bool include_custom_service = false;

        // Wrapper for notifying tests of events
        testing::TestWrapper* test_wrapper{nullptr};

        // BSP values
        types::board_support_common::Event bsp_state; // Current BSP state
        types::board_support_common::Event cp_state;  // Current control pilot state
    } test_data;
