...
iso2_responseCodeType IsoServerBase::iso_validate_state(const int state, const V2gMsgTypeId current_v2g_msg,
                                                        const bool is_dc_charging) {
...
v2g_event IsoServerBase::iso_validate_response_code(iso2_responseCodeType* const v2g_response_code,
                                                    const v2g_connection* const conn) {
...
void IsoServerBase::populate_ac_evse_status(v2g_context* ctx, iso2_AC_EVSEStatusType* evse_status) {
...
void IsoServerBase::check_iso2_charging_profile_values(const iso2_PowerDeliveryReqType* req,
                                                       const iso2_PowerDeliveryResType* res,
                                                       const v2g_connection* conn,
                                                       const uint8_t sa_schedule_tuple_idx) {
...
void IsoServerBase::publish_DcEvStatus(v2g_context* ctx, const iso2_DC_EVStatusType& iso2_ev_status) {
...
iso2_DC_EVSEStatusCodeType IsoServerBase::get_emergency_status_code(const v2g_context* ctx, const uint8_t phase_type) {
...
void IsoServerBase::publish_iso_service_discovery_req(
    const iso2_ServiceDiscoveryReqType* const v2g_service_discovery_req) {
    // V2G values that can be published: ServiceCategory, ServiceScope
}

void IsoServerBase::publish_iso_service_detail_req(const iso2_ServiceDetailReqType* const v2g_service_detail_req) {
    // V2G values that can be published: ServiceID
}

void IsoServerBase::publish_iso_payment_service_selection_req(
    const iso2_PaymentServiceSelectionReqType* const v2g_payment_service_selection_req) {
    // V2G values that can be published: selected_payment_option, SelectedServiceList
}

void IsoServerBase::publish_iso_authorization_req(const iso2_AuthorizationReqType* const v2g_authorization_req) {
    // V2G values that can be published: Id, Id_isUsed, GenChallenge, GenChallenge_isUsed
}

void IsoServerBase::publish_iso_charge_parameter_discovery_req(
    v2g_context* ctx,
    const iso2_ChargeParameterDiscoveryReqType* const v2g_charge_parameter_discovery_req) {
...
void IsoServerBase::publish_iso_pre_charge_req(v2g_context* ctx,
                                               const iso2_PreChargeReqType* const v2g_precharge_req) {
...
void IsoServerBase::publish_iso_power_delivery_req(v2g_context* ctx,
                                                   const iso2_PowerDeliveryReqType* const v2g_power_delivery_req) {
...
void IsoServerBase::publish_iso_current_demand_req(v2g_context* ctx,
                                                   const iso2_CurrentDemandReqType* const v2g_current_demand_req) {
...
void IsoServerBase::publish_iso_metering_receipt_req(const iso2_MeteringReceiptReqType* const v2g_metering_receipt_req) {
    // TODO: publish PnC only
}

void  IsoServerBase::publish_iso_welding_detection_req(
    v2g_context* ctx, const iso2_WeldingDetectionReqType* const v2g_welding_detection_req) {
...
bool IsoServerBase::publish_iso_certificate_installation_exi_req(const v2g_context* ctx,
                                                                 const uint8_t* AExiBuffer,
                                                                 const size_t AExiBufferSize) {
    // PnC only
...
v2g_event IsoServerBase::handle_iso_session_setup(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_service_discovery(v2g_connection* conn) {
    ...
    // Checking of the payment option constraints
    iso2_paymentOptionType payment_option_list[iso2_paymentOptionType_2_ARRAY_SIZE];
    int payment_option_list_len = 0;

    for (int i = 0; i < conn->ctx->evse_v2g_data.payment_option_list_len; i++) {
        const auto payment_option = conn->ctx->evse_v2g_data.payment_option_list[i];

        if (payment_option == iso2_paymentOptionType_ExternalPayment) {
            if (conn->ctx->test_data.auth_eim_supported) {
                payment_option_list[payment_option_list_len++] = payment_option;
            } else {
                dlog(DLOG_LEVEL_WARNING,
                     "ExternalPayment is not allowed by test constraints. Excluding value from payment options.");
            }
        } else if (payment_option == iso2_paymentOptionType_Contract) {
            if (conn->ctx->test_data.auth_pnc_supported) {
                payment_option_list[payment_option_list_len++] = payment_option;
            } else {
                dlog(DLOG_LEVEL_WARNING,
                     "PnC is not allowed by test constraints. Excluding value from payment options.");
            }
        }
    }

    // Copy local payment option list to V2G context
    memcpy(conn->ctx->evse_v2g_data.payment_option_list, payment_option_list,
           payment_option_list_len * sizeof(iso2_paymentOptionType));
    conn->ctx->evse_v2g_data.payment_option_list_len = payment_option_list_len;

    // Copy V2G context payment option list to response message
    ...
    // FAIL the response if test constraints excluded all valid payment options
    if (conn->ctx->evse_v2g_data.payment_option_list_len == 0) {
        res->ResponseCode = iso2_responseCodeType_FAILED;
        conn->ctx->test_data.errors.emplace_back("Test constraints exclude all available payment options");
        dlog(DLOG_LEVEL_ERROR, "Test constraints excluded all available payment options");
    }
...
v2g_event IsoServerBase::handle_iso_service_detail(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_payment_service_selection(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_payment_details(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_authorization(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_charge_parameter_discovery(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_power_delivery(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_charging_status(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_metering_receipt(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_certificate_update(v2g_connection* conn) {
    // TODO: implement CertificateUpdate handling
    return V2G_EVENT_NO_EVENT;
}

v2g_event IsoServerBase::handle_iso_certificate_installation(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_cable_check(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_pre_charge(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_current_demand(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_welding_detection(v2g_connection* conn) {
...
v2g_event IsoServerBase::handle_iso_session_stop(v2g_connection* conn) {
...
V2gMsgTypeId IsoServerBase::find_req_message_type(const v2g_connection* conn) {
    const auto msg_in = conn->exi_in.iso2EXIDocument->V2G_Message.Body;
    auto message_type = V2G_UNKNOWN_MSG;

    if (msg_in.CurrentDemandReq_isUsed) {
        message_type = V2G_CURRENT_DEMAND_MSG;
    } else if (msg_in.SessionSetupReq_isUsed) {
        message_type = V2G_SESSION_SETUP_MSG;
    } else if (msg_in.ServiceDiscoveryReq_isUsed) {
        message_type = V2G_SERVICE_DISCOVERY_MSG;
    } else if (msg_in.ServiceDetailReq_isUsed) {
        message_type = V2G_SERVICE_DETAIL_MSG;
    } else if (msg_in.PaymentServiceSelectionReq_isUsed) {
        message_type = V2G_PAYMENT_SERVICE_SELECTION_MSG;
    } else if (msg_in.PaymentDetailsReq_isUsed) {
        message_type = V2G_PAYMENT_DETAILS_MSG;
    } else if (msg_in.AuthorizationReq_isUsed) {
        message_type = V2G_AUTHORIZATION_MSG;
    } else if (msg_in.ChargeParameterDiscoveryReq_isUsed) {
        message_type = V2G_CHARGE_PARAMETER_DISCOVERY_MSG;
    } else if (msg_in.PowerDeliveryReq_isUsed) {
        message_type = V2G_POWER_DELIVERY_MSG;
    } else if (msg_in.ChargingStatusReq_isUsed) {
        message_type = V2G_CHARGING_STATUS_MSG;
    } else if (msg_in.MeteringReceiptReq_isUsed) {
        message_type = V2G_METERING_RECEIPT_MSG;
    } else if (msg_in.CertificateUpdateReq_isUsed) {
        message_type = V2G_CERTIFICATE_UPDATE_MSG;
    } else if (msg_in.CertificateInstallationReq_isUsed) {
        message_type = V2G_CERTIFICATE_INSTALLATION_MSG;
    } else if (msg_in.CableCheckReq_isUsed) {
        message_type = V2G_CABLE_CHECK_MSG;
    } else if (msg_in.PreChargeReq_isUsed) {
        message_type = V2G_PRE_CHARGE_MSG;
    } else if (msg_in.WeldingDetectionReq_isUsed) {
        message_type = V2G_WELDING_DETECTION_MSG;
    } else if (msg_in.SessionStopReq_isUsed) {
        message_type = V2G_SESSION_STOP_MSG;
    }

    return message_type;
}

v2g_event IsoServerBase::handle_request(v2g_connection* conn) {
    ...
    enum V2gMsgTypeId v2g_req_type = V2G_UNKNOWN_MSG;
    ...
    v2g_req_type = find_req_message_type(conn);
