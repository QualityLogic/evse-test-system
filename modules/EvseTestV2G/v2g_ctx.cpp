...
void v2g_ctx_init_testing_values(v2g_context* const ctx) {
    ctx->test_data.supported_protocols = static_cast<unsigned int>(0);
    ctx->test_data.outcome = types::evse_test_common::TestOutcome::NotStarted;
    ctx->test_data.evidence.clear();
    ctx->test_data.reports.clear();
    ctx->test_data.errors.clear();
    ctx->force_next_connection_req_res = false;
}
...
struct v2g_context* v2g_ctx_create(ISO15118_chargerImplBase* p_chargerImplBase,
    ...
    // TODO There are c++ objects within v2g_context and calloc doesn't call initializers.
    ...
    pthread_mutex_init(&ctx->mqtt_lock, nullptr);
