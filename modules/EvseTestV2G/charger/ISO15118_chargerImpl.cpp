...
void ISO15118_chargerImpl::handle_session_setup(std::vector<types::iso15118::PaymentOption>& payment_options,
    ...
    bool external_payment_available = false;
    ...
    if (external_payment_available) {
        if (v2g_ctx->test_data.auth_eim_supported and not v2g_ctx->test_data.auth_pnc_supported) {
            dlog(DLOG_LEVEL_INFO, "Publishing auth token for ExternalPayment authorization.");
            mod->signal_new_token();
        } else if (not v2g_ctx->test_data.auth_eim_supported) {
            dlog(DLOG_LEVEL_INFO, "Suppressed auth token because EIM is not supported by current test config.");
        } else if (v2g_ctx->test_data.auth_pnc_supported) {
            dlog(DLOG_LEVEL_INFO, "Suppressed auth token because PnC is supported by current test config.");
        }
    } else {
        dlog(DLOG_LEVEL_INFO, "Suppressed auth token because ExternalPayment was not included in session setup payment options.");
    }
    ...
    v2g_ctx->test_data.include_custom_service = mod->config.include_custom_service;
...
void ISO15118_chargerImpl::handle_update_meter_info(types::powermeter::Powermeter& powermeter) {
    ...
    if (test_wrapper != nullptr) {
        test_wrapper->dispatch_powermeter_event(now, powermeter);
    }
}
